 ...
 */
public class LChip extends Chip{
    public int position = 0;

    public LChip(RelativeLayout layout, boolean[][] board, int x, int y, int brickSize, Context context, boolean next, GameOverListener gameOver) {
        if(next){
            for (int i = 0; i < 3; i++) {
                new Brick(layout, R.drawable.blue, x, y+i, brickSize, context);
            }
            new Brick(layout, R.drawable.blue, x+1, y+2, brickSize, context);
        }else{
            if((!board[x][y] && !board[x][y+1] && !board[x][y+2] && !board[x+1][y+2]) || next){
                for (int i = 0; i < 3; i++) {
                    new Brick(layout, R.drawable.blue, x, y+i, brickSize, context);
                }
                new Brick(layout, R.drawable.blue, x+1, y+2, brickSize, context);
                ...
                if(!board[x][y] && !board[x][y+1]&& !board[x+1][y+1]){
                    ...
                    new Brick(layout, R.drawable.blue, x+1, y+1, brickSize, context);
                }else if(!board[x][y]&& !board[x+1][y]){
                    new Brick(layout, R.drawable.blue, x, y, brickSize, context);
                    new Brick(layout, R.drawable.blue, x+1, y, brickSize, context);
                }
    ...
    public void rotate(boolean[][] board, RelativeLayout layout, int brickSize, Context context) {
        ...
        switch(position){
            case 0: //si se sale de la pantalla por la izquierda, se posiciona la ficha al inicio de la fila si no estan ocupadas las posiciones
                if(x4 - 1 < 0){
                    if(!board[x4 + 2][y4] && !board[x3 + 1][y3 - 1]){
                        ...
                        layout.getChildAt(children-2).setX(x2*brick);layout.getChildAt(children-2).setY((y2 - 2) * brick);
                    ...
                    if(!board[x4 + 1][y4] && !board[x2 - 1][y2 - 2] && !board[x1 - 2][y1 - 1]){
                        ...
                        layout.getChildAt(children-2).setX((x2-1)*brick);layout.getChildAt(children-2).setY((y2 - 2) * brick);
                        layout.getChildAt(children-1).setX((x1-2)*brick);layout.getChildAt(children-1).setY((y1 - 1) * brick);
                        position=1;
                        //sino si no estan libres las dos posiciones que ocupa el lado izquierdo de la ficha
                    }else if(board[x2 - 1][y2-2] || board[x1 - 2][y1-1]){
                        //si corriendo un lado a la derecha no me salgo de la pantalla
                        if(x4 + 2 <= Constants.PLAY_AREA_WIDTH - 1){
                            if(!board[x4+2][y4]){
                                layout.getChildAt(children-4).setX((x4 + 2) * brick);layout.getChildAt(children-4).setY(y4 * brick);
                                layout.getChildAt(children-3).setX((x3+1)*brick);layout.getChildAt(children-3).setY((y3 - 1) * brick);
                                layout.getChildAt(children-2).setX(x2*brick);layout.getChildAt(children-2).setY((y2 - 2) * brick);
                ...
                break;
            case 1: //en este if se verifica que la nueva posicion no se salga de la pantalla
                if(y4 + 2 < Constants.PLAY_AREA_HEIGHT){
                    if(x1!=0){// esto es para que si la ficha esta pegada al lado izquierdo, al girarla siga pegada
                        ...
                        if(!board[x4][y4 + 2] && !board[x3 + 1][y3 + 1]){
                            layout.getChildAt(children-4).setX(x4 * brick);layout.getChildAt(children-4).setY((y4 + 2) * brick);
                            layout.getChildAt(children-3).setX((x3+1)*brick);layout.getChildAt(children-3).setY((y3 + 1) * brick);
                            layout.getChildAt(children-2).setX((x2+2)*brick);layout.getChildAt(children-2).setY(y2 * brick);
                            layout.getChildAt(children-1).setX((x1+1)*brick);layout.getChildAt(children-1).setY((y1 - 1) * brick);
                            position=2;
                        }
                    }else{
                        //en este if se verifica que la nueva posicion no se solape con otra ficha
                        if(!board[x4-1][y4 + 2] && !board[x3][y3 + 1]){
                            layout.getChildAt(children-4).setX((x4 - 1) * brick);layout.getChildAt(children-4).setY((y4 + 2) * brick);
                            layout.getChildAt(children-3).setX(x3*brick);layout.getChildAt(children-3).setY((y3 + 1) * brick);
                ...
                break;
            case 2: //si se sale de la pantalla por la izquierda, se posiciona la ficha al inicio de la fila si no estan ocupadas las posiciones
                if(x4 - 2 < 0){
                    if(!board[x4 - 1][y4-1] && !board[x2 +1][y2 +1] && !board[x1+2][y1]){
                ...
                //entonces no sale de la pantalla
                    ...
                    //si al girarla no estan ocupadas las posiciones se ejecuta el giro
                    if(!board[x4 -2][y4-1] && !board[x3-1][y3]){
                        layout.getChildAt(children-4).setX((x4 - 2) * brick);layout.getChildAt(children-4).setY((y4 - 1) * brick);
                ...
                break;
            case 3: //en este if se verifica que la nueva posicion no se salga de la pantalla
                if(y1 + 2 < Constants.PLAY_AREA_HEIGHT){
                    if(x4!=0){// esto es para que si la ficha esta pegada al lado izquierdo, al girarla siga pegada
                        //en este if se verifica que la nueva posicion no se solape con otra ficha
                        if(!board[x4 + 1][y4 - 1] && !board[x2 - 1][y2 + 1] && !board[x1][y1 + 2]){
                            layout.getChildAt(children-4).setX((x4 + 1) * brick);layout.getChildAt(children-4).setY((y4 - 1) * brick);
                            layout.getChildAt(children-2).setX((x2-1)*brick);layout.getChildAt(children-2).setY((y2 + 1) * brick);
                        ...
                        //en este if se verifica que la nueva posicion no se solape con otra ficha
                        if(!board[x4][y4 - 1] && !board[x2 - 2][y2 + 1] && !board[x1-1][y1 + 2]){
                            layout.getChildAt(children-4).setX(x4*brick);layout.getChildAt(children-4).setY((y4-1)*brick);
                            layout.getChildAt(children-3).setX((x3-1)*brick);layout.getChildAt(children-3).setY(y3* brick);
                            layout.getChildAt(children-2).setX((x2-2)*brick);layout.getChildAt(children-2).setY((y2+1) * brick);
